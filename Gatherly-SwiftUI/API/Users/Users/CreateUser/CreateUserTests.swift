...
final class CreateUserTests: XCTestCase {
    ...
    func testCreateUser_incrementsIDAndPersists() {
        let expectation = XCTestExpectation(
            description: "Two users are created and persisted with unique IDs"
        )
        ...
        GatherlyAPI.createUser(user1)
            ...
            .sink { secondCreated in
                ...
                XCTAssertEqual(storedDict.count, 2)
                
                let ids = Array(storedDict.keys)
    ...
    func testCreateUserFromContact_persistsCorrectly() async {
        ...
        let storedDict = UserDefaultsManager.loadUsers()
        XCTAssertTrue(storedDict.keys.contains(99))
    }
}
